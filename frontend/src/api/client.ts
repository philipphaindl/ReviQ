// API client — all functions return unwrapped response data.
...
export const setVenueCategory = (pid: number, venue: string, category: string | null) =>
  api.patch<{ updated: number }>(`/projects/${pid}/papers/venue-category`, { venue, category }).then(r => r.data)

export const enrichVenues = (pid: number) =>
  api.post<{ updated: number; skipped: number; errors: number; total_candidates: number }>(
    `/projects/${pid}/papers/enrich-venues`
  ).then(r => r.data)
...
export const getSearchMetrics = (pid: number) =>
  api.get<{
  ...
  }>(`/projects/${pid}/export/search-metrics`).then(r => r.data)
...
export const exportReplicationPackageUrl = (pid: number) =>
  `/api/projects/${pid}/replication/export`

export const importReplicationPackage = (file: File) => {
  const formData = new FormData()
  formData.append('file', file)
  return api.post<{ id: number; title: string; message: string }>(
    '/projects/replication/import',
    formData,
    { headers: { 'Content-Type': 'multipart/form-data' } },
  ).then(r => r.data)
}
