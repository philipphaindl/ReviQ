/**
 * Screening (Phase 3) — Title/abstract screening with Include/Exclude/Uncertain decisions,
 * inter-reviewer conflict resolution, and Cohen's kappa agreement statistics.
 */
...
export default function Screening() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-ink font-display">Screening</h1>
        <p className="text-sm text-ink-muted">Phase 3 — Title and Abstract Screening</p>
      </div>
...
function PapersView({ pid }: { pid: number }) {
  ...
  // Filter papers by the ACTIVE reviewer's own decision — switching the
  // reviewer in the top bar switches queue, badges, and counts.
  ...
  const counts = {
    ...
    undecided: originalPapers.filter(p => !myDecisionOf(p)).length,
    I: originalPapers.filter(p => myDecisionOf(p) === 'I').length,
    E: originalPapers.filter(p => myDecisionOf(p) === 'E').length,
    U: originalPapers.filter(p => myDecisionOf(p) === 'U').length,
    consensus: originalPapers.filter(p => consensusDecision(p) !== null).length,
  }
  ...
  return (
    <div className="space-y-4">
      {/* Stats */}
      <StatBar>
        <StatCell label="Total" value={counts.total} sub="After deduplication" />
        ...
        <StatCell label="Consensus" value={counts.consensus} sub="Final → Phase 4 (Eligibility)" />
      </StatBar>
...
function PaperRow({ paper, reviewerId, onDecide }: { paper: Paper; reviewerId?: number; onDecide: () => void }) {
  const myDec = ownDecision(paper, reviewerId)
  ...
  // Consensus is only surfaced when it adds information beyond the reviewer's
  // own vote; a provisional (single-vote) final stays hidden to keep screening
  // independent.
  const consensus = consensusDecision(paper)
  const conflict = hasOpenConflict(paper)

  return (
    <div className={`paper-row ${accentClass}`} onClick={onDecide}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-0.5">
          ...
          <span className="text-2xs text-ink-muted ml-auto">{paper.source} · {paper.year}</span>
        </div>
        <p className="text-sm font-medium text-ink leading-snug">{paper.title}</p>
        {paper.authors && <p className="text-2xs text-ink-muted mt-0.5 truncate">{formatAuthors(paper.authors)}</p>}
      </div>
    </div>
  )
}
...
function DecisionModal({
  ...
  // Uncertain decisions require a "leaning toward" direction (I or E) so a criterion
  // from the correct list can be attached, plus a mandatory rationale comment.
  ...
  return (
    <Modal title="Screening Decision" onClose={onClose} width="max-w-2xl" onEnter={handleSubmit}>
      ...
      <div className="bg-paper rounded-md p-4 mb-4 border border-rule">
        ...
        <p className="text-xs text-ink-muted mb-3">{formatAuthors(paper.authors)} · {paper.year} · {paper.source}</p>
        {paper.abstract ? (
          <div className="border-t border-rule pt-3">
            <p className="text-xs font-semibold text-ink-muted uppercase tracking-label mb-1.5">Abstract</p>
            <p className="text-xs text-ink-light leading-relaxed overflow-y-auto max-h-64">{paper.abstract}</p>
          </div>
        ) : (
...
function ConflictsView({ pid }: { pid: number }) {
  ...
  return (
    <div className="space-y-4">
      <StatBar>
        <StatCell label="Open Conflicts" value={conflicts.length} color={conflicts.length > 0 ? 'exclude' : 'navy'} />
        <StatCell label="Resolved" value={resolved.length} color="include" />
      </StatBar>
      ...
      {selected && (
        <Modal title="Resolve Conflict" onClose={() => setSelected(null)} width="max-w-xl">
          <div className="mb-4">
            <p className="text-sm font-semibold text-ink mb-0.5">{selected.paper_title}</p>
            <p className="text-xs text-ink-muted">{selected.paper_citekey}</p>
          </div>
            ...
            {[
              ...
              <div key={r.role} className={`p-3 rounded-md border ${r.decision === 'I' ? 'border-green-200 bg-green-50' : r.decision === 'E' ? 'border-red-200 bg-red-50' : 'border-orange-200 bg-orange-50'}`}>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-bold text-ink-light">{r.role}</span>
                  {r.decision && <DecisionBadge decision={r.decision} />}
                </div>
                {r.rationale && <p className="text-xs text-ink-light">{r.rationale}</p>}
              </div>
            ))}
          </div>
  ...
  return (
    <div className={`p-3 rounded-md border ${resolved ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-ink truncate">{conflict.paper_title ?? conflict.paper_citekey}</p>
          <div className="flex items-center gap-3 mt-1">
            <span className="text-xs text-ink-muted">{r1Name}:</span>
            {conflict.r1_decision && <DecisionBadge decision={conflict.r1_decision} />}
            <span className="text-xs text-ink-muted">vs.</span>
            <span className="text-xs text-ink-muted">{r2Name}:</span>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {conflict.resolution_method && (
                <span className="text-xs text-ink-muted italic">
                  {({ discussion: 'via discussion', arbitration: 'by arbitration', agreement: 'by agreement' } as Record<string, string>)[conflict.resolution_method] ?? conflict.resolution_method}
                </span>
              )}
            </div>
          )}
        ...
        {!resolved && onResolve && (
          <button className="btn-primary shrink-0" onClick={onResolve}>Resolve</button>
        )}
      </div>
    </div>
  )
    ...
    <div className="max-w-xl space-y-4">
      <Card>
        ...
        {kappa && (
          <div className="space-y-4">
            <StatBar>
              <StatCell label="Cohen's κ" value={kappa.kappa.toFixed(3)}
                sub={`95% CI [${kappa.kappa_ci_lower.toFixed(3)}, ${kappa.kappa_ci_upper.toFixed(3)}]`}
                color={kappaColor(kappa.kappa)} />
              <StatCell label="PABAK" value={kappa.pabak.toFixed(3)}
                sub="Prevalence & bias adjusted" />
            </StatBar>

            <div className="bg-paper rounded-md p-3 border border-rule">
              <p className="text-xs font-semibold text-ink-muted uppercase tracking-label mb-1">Interpretation</p>
              <p className="text-sm font-semibold text-ink">{kappa.interpretation}</p>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-rule">
                ...
                <tr><td className="py-2 text-ink-light">Disagreements</td><td className="py-2 text-right text-uncertain font-medium">{kappa.n_disagree}</td></tr>
                <tr><td className="py-2 text-ink-light">Reviewers</td><td className="py-2 text-right text-ink">{kappa.r1_name} vs. {kappa.r2_name}</td></tr>
              </tbody>
            </table>
          </div>
        )}
      ...
      <Card>
        ...
        <div className="space-y-1.5 text-xs text-ink-light">
          <p><span className="font-semibold text-ink">κ ≥ 0.81</span> — Almost perfect agreement</p>
          <p><span className="font-semibold text-ink">κ 0.61–0.80</span> — Substantial agreement</p>
          <p><span className="font-semibold text-ink">κ 0.41–0.60</span> — Moderate agreement</p>
          <p><span className="font-semibold text-ink">κ 0.21–0.40</span> — Fair agreement</p>
          <p><span className="font-semibold text-ink">κ &lt; 0.21</span> — Slight / poor agreement</p>
          <p className="pt-2 text-ink-muted">U (Uncertain) counts as a distinct category, not as an abstention. PABAK adjusts for prevalence and bias (Byrt, T., Bishop, J., & Carlin, J. B. (1993). Bias, prevalence and kappa. <em>Journal of Clinical Epidemiology, 46</em>(5), 423–429).</p>
        </div>
      </Card>
    </div>
  )
