/**
 * Full-Text Eligibility (Phase 4) — Second-pass assessment on screening-included papers.
 * Merges screening decisions with full-text decisions: papers enter from Phase 3 with an
 * "Included" screening verdict, then get a separate full-text Include/Exclude/Uncertain decision.
 */
...
export default function Eligibility() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-ink font-display">Full-Text Eligibility</h1>
        <p className="text-sm text-ink-muted">Phase 4 — Full-Text Eligibility Assessment</p>
      </div>
...
function PapersView({ pid }: { pid: number }) {
  ...
  // Overlay full-text decisions onto screening-included papers (keeps paper metadata
  // from screening, replaces final_decision/decisions with FT values where they exist)
  ...
  // Filter/counts follow the ACTIVE reviewer's own full-text decision, so the
  // reviewer switch in the top bar switches what this page shows.
  ...
  const counts = {
    ...
    undecided: eligiblePapers.filter(p => !myDecisionOf(p)).length,
    I: eligiblePapers.filter(p => myDecisionOf(p) === 'I').length,
    E: eligiblePapers.filter(p => myDecisionOf(p) === 'E').length,
    U: eligiblePapers.filter(p => myDecisionOf(p) === 'U').length,
    consensus: eligiblePapers.filter(p => consensusDecision(p) !== null).length,
  }
  const youSub = activeReviewer ? `by ${activeReviewer.name}` : undefined

  const snowballingEligible = eligiblePapers.filter(p => p.source?.startsWith('snowballing:'))
  const snowballingPending  = snowballingEligible.filter(p => !p.final_decision)

  return (
    <div className="space-y-4">
      <StatBar>
        <StatCell label="Eligible" value={counts.total} sub="Screening-included" />
        ...
        <StatCell label="Consensus" value={counts.consensus} sub="Final → Phase 6/7" />
      </StatBar>

      {snowballingEligible.length > 0 && (
...
function PaperRow({ paper, reviewerId, onDecide }: { paper: Paper; reviewerId?: number; onDecide: () => void }) {
  ...
  const isSnowball = paper.source?.startsWith('snowballing:')
    ...
    <div className={`paper-row ${accentClass}`} onClick={onDecide}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-0.5">
          ...
          {isSnowball && <Badge label="Snowballing" variant="info" />}
        ...
        {paper.full_text_url && (
          <a href={paper.full_text_url} target="_blank" rel="noopener noreferrer"
            className="text-2xs text-info hover:underline mt-0.5 block"
...
function DecisionModal({
  paper, inclusionCriteria, exclusionCriteria, onSubmit, onClose, isPending, error, pid, reviewerId,
}: {
  paper: Paper
  ...
  // co-reviewer's vote and break independent assessment.
  ...
  return (
    <Modal title="Full-Text Eligibility Decision" onClose={onClose} width="max-w-2xl" onEnter={handleSubmit}>
      ...
      <div className="bg-blue-50 rounded-md p-3 mb-4 border border-blue-200 space-y-2">
        <p className="text-xs font-semibold text-ink uppercase tracking-label">Full-Text Access</p>
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          <span className="text-xs text-ink-light">Full text inaccessible</span>
        </label>
        <button className="btn-secondary" onClick={savePaperMeta} disabled={saving}>
        ...
        {kappa && (
          <div className="space-y-4">
            <StatBar>
              ...
              <StatCell label="PABAK" value={kappa.pabak.toFixed(3)} sub="Prevalence & bias adjusted" />
            </StatBar>
