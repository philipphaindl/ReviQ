/**
 * Setup (Phase 1) — Project configuration across six tabs: project details, reviewers,
 * inclusion/exclusion criteria, QA criteria, taxonomy categories, and database search strings.
 */
...
export default function Settings() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-ink font-display">Setup</h1>
        <p className="text-sm text-ink-muted">Phase 1 — Project Configuration and Protocol Setup</p>
      </div>
...
function ProjectTab({ pid }: { pid: number }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      // Keep R1 name in sync with lead researcher
      if (form.lead_researcher) {
        const r1 = reviewers.find(r => r.role === 'R1')
        if (r1 && r1.name !== form.lead_researcher) {
          await updateReviewer(pid, r1.id, { name: form.lead_researcher as string, email: r1.email ?? '' })
          qc.invalidateQueries({ queryKey: ['reviewers', pid] })
        }
      }
      setForm({})
      setSubmitted(false)
...
function ReviewersTab({ pid }: { pid: number }) {
  ...
  const editMutation = useMutation({
    mutationFn: ({ id, data, role }: { id: number; data: { name: string; email: string }; role: string }) => updateReviewer(pid, id, data),
    onSuccess: async (_data, vars) => {
      qc.invalidateQueries({ queryKey: ['reviewers', pid] })
      // If R1 name changed, sync to project lead_researcher
      if (vars.role === 'R1') {
        await updateProject(pid, { lead_researcher: vars.data.name })
        qc.invalidateQueries({ queryKey: ['project', pid] })
      }
      setModal(null)
      setSubmitted(false)
  ...
  const submit = () => {
    ...
    else if (modal?.kind === 'edit') editMutation.mutate({ id: modal.id, data: { name: form.name, email: form.email }, role: modal.role })
  }
      ...
      <Card>
        <CardHeader
          ...
          action={reviewers.length < 5
            ? <button className="btn-secondary" onClick={openAdd}>+ Add</button>
            : null}
        />
        {reviewers.length === 0 ? (
          ...
          <div className="divide-y divide-rule">
            {reviewers.sort((a, b) => a.role.localeCompare(b.role)).map(r => (
              <div key={r.id} className="py-3 flex items-center gap-3">
                <span className={`text-xs font-bold px-2 py-0.5 rounded-sm border shrink-0 ${r.role === 'R1' ? 'bg-accent-faint text-accent border-accent/20' : 'bg-paper text-ink-muted border-rule'}`}>
                ...
                <div className="flex gap-1 shrink-0">
                  ...
                  {r.role !== 'R1' && (
                    <button className="btn-danger" onClick={() => setConfirmDelete(r.id)}>Remove</button>
                  )}
                </div>
              </div>
            ))}
...
function CriteriaTab({ pid }: { pid: number }) {
  ...
  const [form, setForm] = useState({ label: '', description: '', phase: 'screening', short_label: '' })
  const [submitted, setSubmitted] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState<{ type: 'i' | 'e'; id: number } | null>(null)

  const openAdd = (type: 'add-i' | 'add-e') => {
    setForm({ label: '', description: '', phase: 'screening', short_label: '' }); setSubmitted(false); setModal({ type })
  }
  const openEdit = (type: 'edit-i' | 'edit-e', c: any) => {
    setForm({ label: c.label, description: c.description, phase: c.phase, short_label: c.short_label ?? '' }); setSubmitted(false); setModal({ type, id: c.id })
  }
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader title="Inclusion Criteria"
          action={<button className="btn-secondary" onClick={() => openAdd('add-i')}>+ Add</button>} />
        {inclusions.length === 0
      ...
      <Card>
        <CardHeader title="Exclusion Criteria"
          action={<button className="btn-secondary" onClick={() => openAdd('add-e')}>+ Add</button>} />
        {exclusions.length === 0
      ...
      {modal && (
        ...
        >
          ...
          <FormField label="Short label" hint="Displayed in PRISMA diagram (e.g. 'Not peer-reviewed')">
            <input className="input" placeholder="Optional, max 30 chars" maxLength={30}
...
function CriterionRow({ label, description, badge, variant = 'neutral', onEdit, onDelete }: {
  ...
  const badgeClass =
    variant === 'inclusion' ? 'bg-include text-white' :
    variant === 'exclusion' ? 'bg-exclude text-white' :
    'bg-accent text-white'
  return (
    <div className="flex items-start gap-3 py-3 border-b border-rule last:border-0">
      <span className={`shrink-0 text-xs font-bold rounded-full px-3 py-1 mt-0.5 min-w-[3rem] truncate text-center inline-block ${badgeClass}`} title={label}>{label}</span>
      <div className="flex-1 min-w-0">
        <p className="text-sm text-ink">{description}</p>
        {badge && <span className="text-xs text-ink-muted">{badge}</span>}
      </div>
      <div className="flex gap-1 shrink-0">
        <button onClick={onEdit} className="btn-secondary">Edit</button>
        <button onClick={onDelete} className="btn-danger">Remove</button>
      </div>
    </div>
  )
}
    ...
    <div>
      <Card>
        <CardHeader
          ...
          action={<button className="btn-secondary" onClick={openAdd}>+ Add</button>}
        />
        {criteria.length === 0
          ? <EmptyState icon="—" message="No QA criteria defined." />
          : criteria.map(c => (
            <div key={c.id} className="py-3 border-b border-rule last:border-0 flex items-start gap-3">
              <span className="bg-accent text-white text-xs font-bold rounded-full px-3 py-1 shrink-0 mt-0.5 min-w-[3rem] truncate text-center inline-block" title={c.label}>{c.label}</span>
              <div className="flex-1">
                <p className="text-sm text-ink">{c.description}</p>
                <p className="text-xs text-ink-muted mt-0.5">Max score: {c.max_score}</p>
              </div>
              <div className="flex gap-1 shrink-0">
                <button className="btn-secondary" onClick={() => openEdit(c)}>Edit</button>
                <button className="btn-danger" onClick={() => setConfirmDelete(c.id)}>Remove</button>
              </div>
            </div>
          ))}
      </Card>
      ...
      {modal && (
        <Modal title={modal.mode === 'add' ? 'Add Quality Assessment Criterion' : 'Edit Quality Assessment Criterion'}
...
// Default taxonomy categories — merged with whatever the backend already has.
// Deleted defaults are tracked in localStorage so they don't reappear on reload.
...
const PROTECTED_TAXONOMY_KEYS = new Set(['research_type', 'contribution_type'])
    ...
    <div className="max-w-xl space-y-3">
      ...
      <Card>
        <CardHeader
          ...
          action={
            <button className="btn-secondary" onClick={() => { setCatForm({ label: '' }); setCatSubmitted(false); setModal({ kind: 'add-category' }) }}>
              + New Category
            </button>
          }
        />
        <div className="divide-y divide-rule">
          {allTypes.map(t => (
            <div key={t.key} className={`flex items-center justify-between py-2 px-1 rounded cursor-pointer transition-colors ${activeKey === t.key ? 'bg-blue-50' : 'hover:bg-paper'}`}
              onClick={() => setActiveKey(t.key)}>
              <span className={`text-sm font-medium ${activeKey === t.key ? 'text-info' : 'text-ink'}`}>{t.label}</span>
              <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                ...
                {!PROTECTED_TAXONOMY_KEYS.has(t.key) && (
      ...
      <Card>
        <CardHeader
          ...
          action={<button className="btn-secondary" onClick={() => { setEntryValue(''); setEntrySubmitted(false); setModal({ kind: 'add-entry' }) }}>+ Add</button>}
        />
        {entries.length === 0
                  ...
                  {editingEntryId === e.id ? (
                    <div className="flex-1 flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        <button className="btn-primary px-2 py-1 shrink-0"
                          disabled={editEntryMutation.isPending}
                          onClick={() => {
                            if (!editEntryValue.trim()) { setEditEntryError(true); return }
                            editEntryMutation.mutate({ id: e.id, value: editEntryValue.trim() })
                          }}>Save</button>
                        <button className="btn-secondary shrink-0" onClick={() => setEditingEntryId(null)}>Cancel</button>
                      </div>
                    ...
                    <>
                      <span className="text-sm text-ink flex-1">{e.value}</span>
                      <button className="btn-secondary shrink-0" onClick={() => { setEditingEntryId(e.id); setEditEntryValue(e.value); setEditEntryError(false) }}>Edit</button>
                      <button className="btn-danger shrink-0" onClick={() => setModal({ kind: 'confirm-delete-entry', id: e.id })}>Remove</button>
                    </>
                  )}
                </div>
              ))}
...
function SearchStringsTab({ pid }: { pid: number }) {
  ...
  const [form, setForm] = useState<{ db_name: string; query_string: string; filter_settings: string; search_date: string; results_count: string }>({ db_name: DATABASES[0].key, query_string: '', filter_settings: '', search_date: '', results_count: '' })
  const [submitted, setSubmitted] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState<number | null>(null)

  const formPayload = () => ({
    ...form,
    results_count: form.results_count.trim() !== '' ? parseInt(form.results_count, 10) : undefined,
  })

  const addMutation = useMutation({
    mutationFn: () => addSearchString(pid, formPayload()),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['search-strings', pid] }); setModal(null); setSubmitted(false) },
  })
  const updateMutation = useMutation({
    mutationFn: () => updateSearchString(pid, modal!.id!, formPayload()),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['search-strings', pid] }); setModal(null); setSubmitted(false) },
  })
  const delMutation = useMutation({
  ...
  const openAdd = () => {
    ...
    setForm({ db_name: firstAvailable, query_string: '', filter_settings: '', search_date: '', results_count: '' })
    setSubmitted(false)
    setModal({ mode: 'add' })
  }
  const openEdit = (s: any) => {
    setForm({ db_name: s.db_name, query_string: s.query_string ?? '', filter_settings: s.filter_settings ?? '', search_date: s.search_date ?? '', results_count: s.results_count != null ? String(s.results_count) : '' })
    setSubmitted(false)
    ...
    <div>
      <Card>
        ...
        {strings.length === 0
          ...
          : strings.map(s => (
            <div key={s.id} className="py-4 border-b border-rule last:border-0">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex-1 min-w-0">
                  {s.query_string
                    ? <p className="text-xs font-mono text-ink-light bg-paper p-2 rounded break-all leading-relaxed">{s.query_string}</p>
                    : <p className="text-xs text-ink-muted italic">No query string</p>}
                  <div className="flex flex-col gap-0.5 mt-1.5">
                    {s.filter_settings && <span className="text-xs text-ink-muted"><span className="font-semibold text-ink-light">Filter:</span> {s.filter_settings}</span>}
                    {s.search_date && <span className="text-xs text-ink-muted"><span className="font-semibold text-ink-light">Search Date:</span> {s.search_date}</span>}
                    {s.results_count != null && <span className="text-xs text-ink-muted"><span className="font-semibold text-ink-light">Results:</span> {s.results_count.toLocaleString()}</span>}
                  </div>
                </div>
                <div className="flex gap-1 shrink-0">
                  <button className="btn-secondary" onClick={() => openEdit(s)}>Edit</button>
                  <button className="btn-danger" onClick={() => setConfirmDelete(s.id)}>Remove</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {modal && (
        <Modal title={modal.mode === 'add' ? 'Add Search String' : 'Edit Search String'}
          ...
          <FormField label="Results Count" hint="Total hits reported by the database (used as denominator for precision)">
            <input type="number" className="input" min={0} placeholder="e.g. 1234" value={form.results_count}
              onChange={e => setForm(f => ({ ...f, results_count: e.target.value }))} />
          </FormField>
          <button className="btn-primary w-full justify-center mt-2"
