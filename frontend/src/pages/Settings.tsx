...
import { Card, CardHeader, CriterionRow, Modal, FormField, EmptyState } from '../components/ui'
    ...
    <div className="max-w-xl space-y-4">
      <Card>
        ...
        <FormField label="Methodology">
          <input className="input" value={project.methodology} disabled />
        </FormField>
      </Card>
      ...
      <button
        ...
        onClick={() => { mutation.mutate(form); setForm({}) }}
      >
        {mutation.isPending ? 'Saving…' : 'Save Changes'}
      </button>
      {mutation.isSuccess && <span className="text-xs text-include ml-3">Saved</span>}
    </div>
  )
}
...
function ReviewersTab({ pid }: { pid: number }) {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['reviewers', pid] }); setModal(null); setForm({ name: '', email: '', role: 'R2' }) },
  })
    ...
    <div className="max-w-xl">
      <Card>
        <CardHeader
          ...
          action={
            reviewers.length < 5 ? (
              <button className="btn-secondary text-xs" onClick={() => setModal('add')}>+ Add</button>
            ) : null
          }
        />
        {reviewers.length === 0 ? (
          <EmptyState icon="👤" message="No reviewers configured yet. R1 is always the lead reviewer." />
        ) : (
          <div className="divide-y divide-border">
            {reviewers.sort((a, b) => a.role.localeCompare(b.role)).map(r => (
              <div key={r.id} className="py-3 flex items-center gap-3">
                ...
                {r.role !== 'R1' && (
                  <button className="btn-danger text-xs px-2 py-1" onClick={() => deleteMutation.mutate(r.id)}>×</button>
                )}
              </div>
            ))}
          </div>
      ...
      {modal === 'add' && (
        <Modal title="Add Reviewer" onClose={() => setModal(null)}>
          ...
          <button className="btn-primary w-full justify-center mt-2" disabled={!form.name || addMutation.isPending}
...
function CriteriaTab({ pid }: { pid: number }) {
  ...
  const [modal, setModal] = useState<{ type: 'add-i' | 'add-e' | null }>({ type: null })
  ...
  const addI = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['inclusion', pid] }); setModal({ type: null }); setForm({ label: '', description: '', phase: 'screening' }) },
  })
  const addE = useMutation({
    mutationFn: () => addExclusionCriterion(pid, form),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['exclusion', pid] }); setModal({ type: null }); setForm({ label: '', description: '', phase: 'screening' }) },
  })
    ...
    <div className="max-w-2xl space-y-4">
      <Card>
        <CardHeader title="Inclusion Criteria"
          action={<button className="btn-secondary text-xs" onClick={() => setModal({ type: 'add-i' })}>+ Add</button>} />
        {inclusions.length === 0
          ? <EmptyState icon="✅" message="No inclusion criteria defined." />
          : inclusions.map(c => (
            <CriterionRow key={c.id} label={c.label} description={c.description}
              badge={`Phase: ${c.phase}`}
              onEdit={() => {}} onDelete={() => delI.mutate(c.id)} />
          ))}
      </Card>

      <Card>
        <CardHeader title="Exclusion Criteria"
          action={<button className="btn-secondary text-xs" onClick={() => setModal({ type: 'add-e' })}>+ Add</button>} />
        {exclusions.length === 0
          ? <EmptyState icon="❌" message="No exclusion criteria defined." />
          : exclusions.map(c => (
            <CriterionRow key={c.id} label={c.label} description={c.description}
              badge={`Phase: ${c.phase}`}
              onEdit={() => {}} onDelete={() => delE.mutate(c.id)} />
          ))}
      </Card>

      {modal.type && (
        <Modal title={modal.type === 'add-i' ? 'Add Inclusion Criterion' : 'Add Exclusion Criterion'}
          onClose={() => setModal({ type: null })}>
          <FormField label="Label (e.g. I1 or E3)">
            <input className="input" placeholder="I1" value={form.label}
          ...
          <button className="btn-primary w-full justify-center mt-2"
            disabled={!form.label || !form.description || addI.isPending || addE.isPending}
            onClick={() => modal.type === 'add-i' ? addI.mutate() : addE.mutate()}>
            Add Criterion
          </button>
        </Modal>
      )}
...
function QATab({ pid }: { pid: number }) {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['qa', pid] }); setModal(false); setForm({ label: '', description: '', max_score: 1.0 }) },
  })
    ...
    <div className="max-w-2xl">
      <Card>
        <CardHeader
          ...
          action={<button className="btn-secondary text-xs" onClick={() => setModal(true)}>+ Add</button>}
        />
        {criteria.length === 0
          ...
          : criteria.map(c => (
            <div key={c.id} className="py-3 border-b border-border last:border-0 flex items-start gap-3">
              ...
              <button className="btn-danger text-xs px-2 py-1 shrink-0" onClick={() => delMutation.mutate(c.id)}>×</button>
            </div>
          ))}
      </Card>

      {modal && (
        <Modal title="Add QA Criterion" onClose={() => setModal(false)}>
          <FormField label="Label (e.g. QA1)">
            <input className="input" placeholder="QA1" value={form.label}
              onChange={e => setForm(f => ({ ...f, label: e.target.value }))} />
          </FormField>
          ...
          <button className="btn-primary w-full justify-center mt-2"
            disabled={!form.label || !form.description || addMutation.isPending}
            onClick={() => addMutation.mutate()}>
            Add Criterion
          </button>
        </Modal>
...
const TAXONOMY_TYPES = [
  { key: 'research_type', label: 'Research Type', defaults: ['Experience Paper', 'Evaluation Research', 'Validation Research', 'Philosophical Paper', 'Solution Proposal', 'Opinion Paper'] },
  { key: 'contribution_type', label: 'Contribution Type', defaults: ['Model', 'Method', 'Theory', 'Framework', 'Guideline', 'Lessons Learned', 'Advice', 'Tool'] },
  { key: 'usage_scenario', label: 'Usage Scenario', defaults: [] },
]
...
function TaxonomiesTab({ pid }: { pid: number }) {
  ...
  const currentType = TAXONOMY_TYPES.find(t => t.key === activeType)!
  ...
  return (
    <div className="max-w-xl">
      ...
      <Card>
        <CardHeader title={currentType.label} />
        {entries.length === 0
          ? <p className="text-xs text-gray-400 py-2">No entries. Add below or load defaults.</p>
          : (
            <div className="divide-y divide-border mb-4">
              {entries.map(e => (
                <div key={e.id} className="py-2 flex items-center justify-between">
                  <span className="text-sm text-navy">{e.value}</span>
                  <button className="btn-danger text-xs px-2 py-1" onClick={() => delMutation.mutate(e.id)}>×</button>
                </div>
              ))}
            </div>
          )}
        <div className="flex gap-2">
          ...
          <button className="btn-primary shrink-0" disabled={!newValue || addMutation.isPending}
...
function SearchStringsTab({ pid }: { pid: number }) {
  ...
  return (
    <div className="max-w-2xl">
            ...
            <div key={s.id} className="py-3 border-b border-border last:border-0">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex gap-1">
                  ...
                  <button className="btn-danger text-xs px-2 py-1" onClick={() => delMutation.mutate(s.id)}>×</button>
                </div>
              </div>
      ...
      {modal && (
        <Modal title={modal.mode === 'add' ? 'Add Search String' : 'Edit Search String'} onClose={() => setModal(null)} width="max-w-2xl">
          <FormField label="Database Name">
            <input className="input" placeholder="e.g. scopus, ieee, acm, dblp" value={form.db_name}
          ...
          <button className="btn-primary w-full justify-center mt-2"
            disabled={!form.db_name || addMutation.isPending || updateMutation.isPending}
            onClick={() => modal.mode === 'add' ? addMutation.mutate() : updateMutation.mutate()}>
