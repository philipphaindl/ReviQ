/** Academic database registry — canonical list, key normalization, and badge rendering. */
...
export function DatabaseBadge({ dbKey, size = 'md', block = false, width }: { dbKey: string; size?: 'sm' | 'md' | 'lg'; block?: boolean; width?: string | number }) {
  ...
  return (
    <span
      title={label}
      style={{
        display: (block || width) ? 'flex' : 'inline-flex',
        width: width ?? (block ? '100%' : undefined),
        justifyContent: (block || width) ? 'center' : undefined,
        alignItems: 'center',
